...
import { createCostCenter, updateCostCenter } from '@/api/ledger'
...
async function submit() {
  ...
  if (!form.name.trim()) {
    errors.name = t('ledger.cc.errName')
    return
  }
  saving.value = true
  try {
    isEdit.value ? await updateCostCenter(props.center.id, { ...form }) : await createCostCenter({ ...form })
...
<template>
  <Dialog :open="open" :title="isEdit ? t('ledger.cc.editTitle') : t('ledger.cc.addTitle')" @update:open="emit('update:open', $event)">
    <form class="space-y-4" @submit.prevent="submit">
      <div class="space-y-1.5">
        <label class="text-sm font-medium">{{ t('ledger.cc.name') }}</label>
