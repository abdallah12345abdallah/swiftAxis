...
/** Totals from the invoice amount before tax; VAT is always added on top.
    (Order count, order price and pre-tax extras were dropped from the form —
    the invoice is now entered as one pre-tax amount, taken from the sheet.) */
export function computeSalesTotals({ amount }) {
  const preTax = round2(Number(amount) || 0)
...
export async function createSalesInvoice({ contract, period, date, amount, sheet = null, notes = '' }) {
  if (!contractById(contract)) return Promise.reject(new Error('CONTRACT_REQUIRED'))
  const totals = computeSalesTotals({ amount })
...
export async function markInvoicePaid(id, { treasuryId, date }) {
  ...
  const voucher = await createReceipt({
    treasuryId, date, amount: inv.total, party: contractById(inv.contract)?.company ?? inv.contract, partyType: 'customer', partyId: inv.contract,
...
/** Mock parse of an uploaded partner sheet → prefilled invoice fields
    (`amount` = the sheet's pre-tax total; `rows` = orders found). */
export function parseSalesSheet(file) {
  const name = (file?.name || '').toLowerCase()
  const contract = name.includes('jahez') || name.includes('jz') ? 'jahez' : 'hunger'
  const now = new Date()
  const period = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`
  return mockDelay({ contract, period, amount: 3410 * 12, rows: 3410, riders: 7 }, 600)
}
